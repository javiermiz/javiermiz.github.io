...
I had Claude, Anthropic's model, and I was paying my monthly fee like anyone who wants to use AI seriously. But I quickly ran into the problem we all know: message limits, usage restrictions, and that feeling of paying for something you never fully get to use. It's not that Claude is bad — it's that the subscription model puts an artificial ceiling on you. You reach a certain point and the system tells you "that's enough for today."
...
And at first, honestly, I didn't understand anything. There were tools like **Ollama**, which is basically the simplest way to download and run local models — you type a command in the terminal and seconds later you have a model running. Also **LM Studio** and **Cherry Studio**, which are graphical interfaces to do the same thing if you don't want to touch the terminal. A ton of tools, each one promising to make it easier.

I dove into that world, and right when I started, Alibaba had just released **Qwen 3.6**, a model that fit perfectly on my MacBook Pro M4 Pro with 48GB of RAM. I installed it, tested it, and watching a model answer questions on my own machine, with no internet, felt like magic. I added **MCP** (Model Context Protocol) — a standard that lets models connect with external tools like files, databases, or whatever you need. The possibilities opened up significantly.
...
I discovered **OpenCLAW**, a more advanced framework for building agentic systems. I set it up, ran it, and it worked... painfully slow. It was overkill for my laptop.
...
That's when I found **pi.dev**. And this actually changed things.
...
That's when **Hermes Agent** came in. It's a framework that lets you have assistants with personality, connected to Telegram, capable of executing code, researching the web, controlling files — all from a chat. I had it configured in no time — the system itself guided me through it — and within a while I had **3 Telegram bots** running, each with its own purpose and distinct personality. One for research, one to help with technical tasks, one just to experiment.
...
**Hermes** manages projects, writes plans, and keeps long-term context — and for that it uses Deepseek. It's ideal for tasks that require memory, understanding the full context of a project, and coordinating multiple steps.

**pi.dev** I use for programming and running code quickly. Its short system prompt and ability to parallelize tasks make it much more efficient for that. For pure development, it's still my main tool.
